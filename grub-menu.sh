...
#kernel_version="5.4.0-88"

#kernlist=$(grep -i "menuentry '" /boot/grub/grub.cfg|sed -r "s|--class .*$||g")
#menuline=$(echo "$kernlist" | grep -ne $kernel_version | grep -v recovery | cut -f1 -d":")
#menunum=$(($menuline-2))

#echo "Boot menu list:"
#echo "$(echo "$kernlist" | grep -ne "")"
#echo "kernel $kernel_version menu entry num: $menuline - 2 = $menunum"
